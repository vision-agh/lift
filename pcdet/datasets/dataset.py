...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        if (self.voxel_size is None) and hasattr(self.dataset_cfg, 'VOXEL_SIZE'):
            self.voxel_size = np.array(self.dataset_cfg.VOXEL_SIZE, dtype=np.float32)
        if self.grid_size is None:
            ...
            self.grid_size = np.floor(self.grid_size)

...
def split_bn_bias_by_name(model: nn.Module):
    "Split the layers in `layer_groups` into batchnorm (`bn_types`) and non-batchnorm groups."
    bn_bias_quant_params = []
    other_params = []
    ...
    for c in model.modules():
        if isinstance(c, bn_types):
            param_ids = [ id(p) for p in c.parameters() ]
            bn_params_ids.extend(param_ids)
    for name, p in model.named_parameters():
        if id(p) in bn_params_ids:
            bn_params.append(p)
        elif name.endswith("bias"):
            bias_params.append(p)
        elif "quant" in name and name.endswith("value"):
            quant_params.append(p)
        else:
            other_params.append(p)

    bn_bias_quant_params = bn_params + bias_params + quant_params

    param_groups = [other_params, bn_bias_quant_params]
    return param_groups
...
class OptimWrapper():
    ...
    @classmethod
    def create_quant_aware(cls, opt_func, lr,
               model, **kwargs):
        "Create an `optim.Optimizer` from `opt_func` with `lr`. Set lr on `layer_groups`."
        param_groups = split_bn_bias_by_name(model)
        opt = opt_func([{'params': params, 'lr': 0} for params in param_groups])
        opt = cls(opt, **kwargs)
        opt.lr, opt.opt_func = [lr], opt_func
        return opt
